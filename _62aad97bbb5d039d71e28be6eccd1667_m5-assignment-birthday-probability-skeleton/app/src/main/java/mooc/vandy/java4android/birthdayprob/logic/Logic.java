...
public class Logic 
    ...
    */
    ...
    private boolean runSimulation(int size, int pass) {
        Random generator = new Random(pass);
        Map birthdays = new HashMap<Integer, Integer>();
        for (int i=0; i<size; i++) {
            int randomBirthday = generator.nextInt(365);
            if (birthdays.get(randomBirthday) == null) {
                birthdays.put(randomBirthday, 1);
            } else {
                return true;
            }
        }
